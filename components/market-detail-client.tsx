...
export default function MarketDetailClient({ market }: MarketDetailClientProps) {
    ...
    const breadcrumbData = {
        ...
        "@type": "BreadcrumbList",
        itemListElement: [
            { "@type": "ListItem", position: 1, name: "Home", item: process.env.NEXT_PUBLIC_SITE_URL || "https://pasarmalam.app" },
            { "@type": "ListItem", position: 2, name: "Markets", item: `${process.env.NEXT_PUBLIC_SITE_URL || "https://pasarmalam.app"}/markets` },
            { "@type": "ListItem", position: 3, name: market.name, item: `${process.env.NEXT_PUBLIC_SITE_URL || "https://pasarmalam.app"}/markets/${market.id}` },
        ],
    };
            ...
            <div className="min-h-screen bg-background">
                ...
                <div className="container mx-auto px-4 py-8">
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                        {/* Main content area with market details */}
                        <div className="lg:col-span-2 space-y-6">
                            ...
                            <div>
                                ...
                                {/* Description */}
                                {market.description && <p className="text-muted-foreground text-lg leading-relaxed mb-6">{market.description}</p>}
                            </div>
                                        ...
                                        <div>
                                            ...
                                            <div className="space-y-2">
                                                <div className="flex items-center gap-2">
                                                    <Badge variant={market.parking.available ? "default" : "secondary"}>{market.parking.available ? t.available : t.notAvailable}</Badge>
                            ...
                            <Card>
                                ...
                                <CardContent>
                                    <div className="space-y-4">
                                        ...
                                        {market.location ? (
                                            <InteractiveMap latitude={market.location.latitude} longitude={market.location.longitude} name={market.name} address={market.address} className="w-full h-80 md:h-64 z-2" />
                                        ) : (
                                        ...
                                        {market.location && (
                                            <div className="flex gap-2">
                                                <Button variant="outline" onClick={handleShowDirections} className="cursor-pointer hover:text-white dark:hover:bg-gray-800">
                        ...
                        <div className="space-y-6">
                                    ...
                                    <CardContent className="space-y-3">
                                        {market.contact.phone && (
                                            <div className="flex items-center gap-2">
                                                ...
                                                <a href={`tel:${market.contact.phone}`} className="text-primary hover:underline">{market.contact.phone}</a>
                                            </div>
                                        )}
                                        {market.contact.email && (
                                            <div className="flex items-center gap-2">
                                                <Mail className="h-4 w-4 text-muted-foreground" />
                                                <a href={`mailto:${market.contact.email}`} className="text-primary hover:underline">{market.contact.email}</a>
                                            </div>
                                        )}
                                    </CardContent>
                                </Card>
                            ...
                            <Card>
                                ...
                                <CardContent className="space-y-3">
                                    {market.location && (
                                        <Button className="w-full" onClick={handleShowDirections}>
                                            <MapPin className="h-4 w-4 mr-2" />
                                            {t.getDirections}
                                        </Button>
                                    )}
                                    <Button variant="outline" className="w-full bg-transparent cursor-pointer hover:text-white dark:hover:bg-gray-800" onClick={async () => {
                                        const url = window.location.href;
                                        if (navigator.share) await navigator.share({ title: market.name, url }); else await navigator.clipboard.writeText(url);
                                    }}>
                            ...
                            {/* Other Markets in the Same State */}
                            <Card>
                                <CardHeader>
                                    <CardTitle>{t.otherMarketsIn} {market.state}</CardTitle>
                                </CardHeader>
                                <CardContent>
                                    <p className="text-sm text-muted-foreground">{t.discoverMoreMarkets}</p>
                                    <Link href={`/markets?state=${encodeURIComponent(market.state)}`}>
                                        <Button variant="outline" className="w-full mt-3 bg-transparent cursor-pointer hover:text-white dark:hover:bg-gray-800">
                                        {language === 'ms' ? `${t.browseStateMarkets} ${t.markets} ${market.state}` : `${t.browseStateMarkets} ${market.state} ${t.markets}`}
                                        </Button>
                                    </Link>
                                </CardContent>
                            </Card>
                ...
                {/* Render the mobile-only directions chooser. `openDirections` will call the
            provided callback only on mobile devices, so the dialog is rendered
            here and opened when needed. */}
