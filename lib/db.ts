...
export async function getMarketById(id: string): Promise<Market | null> {
  const supabase = await createClient();
  const { data, error } = await supabase.from("pasar_malams").select("*").eq("id", id).single();
...
export async function getAllStates(): Promise<string[]> {
  const supabase = await createClient();
  const { data, error } = await supabase.from("pasar_malams").select("state").eq("status", "Active");
